...
if(archive && query) {
  ...
  try {
    const response=await fetch(`${BASE}/search-index.json`);if(!response.ok)throw Error('Search unavailable');
    ...
    document.getElementById('post-list').innerHTML=matches.slice((page-1)*size,page*size).map(p=>`<li><a class="post-row" href="${BASE}/blog/${encodeURIComponent(p.slug)}/"><div class="post-copy"><span class="post-category">${escape(p.categoryLabel)}</span><h2>${escape(p.title)}</h2><p>${escape(p.summary)}</p><div class="post-meta">현농푸드랩 · 원고 예시</div></div><img class="post-thumbnail" src="${BASE}/assets/${encodeURIComponent(p.image)}" alt="${escape(p.alt)}" loading="lazy" width="${p.width}" height="${p.height}"></a></li>`).join('');
